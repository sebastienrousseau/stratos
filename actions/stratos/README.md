...
```yaml
- uses: sebastienrousseau/stratos/actions/stratos@v0.0.13
  with:
...
| `version` | no | matches the pinned ref | Stratos version to install. Override with e.g. `version: 0.0.13`. |
...
`@v0.0.13` pins to a tag. The action and the CLI ship from the same repo,
