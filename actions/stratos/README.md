...
```yaml
- uses: sebastienrousseau/stratos/actions/stratos@v0.0.14
  with:
...
| `version` | no | matches the pinned ref | Stratos version to install. Override with e.g. `version: 0.0.14`. |
...
`@v0.0.14` pins to a tag. The action and the CLI ship from the same repo,
